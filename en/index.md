---
layout: single
title: "PixInsight Scripts"
---

[日本語](../)

Custom scripts and tools for PixInsight.

<div markdown="0">
<div class="script-cards-row">
  <a href="{{ site.baseurl }}/manual-image-solver/en/" class="script-card">
    <img src="{{ site.baseurl }}/assets/images/manual-image-solver/05_stars_registered.jpg" alt="Manual Image Solver">
    <h3>Manual Image Solver</h3>
    <p>A manual plate solving tool. Select stars from a built-in catalog to obtain a WCS solution when automatic solvers fail. No Python required — runs entirely within PixInsight.</p>
  </a>
  <a href="{{ site.baseurl }}/split-image-solver/en/" class="script-card">
    <img src="{{ site.baseurl }}/assets/images/split-image-solver/main-dialog.jpg" alt="Split Image Solver">
    <h3>Split Image Solver</h3>
    <p>Splits wide-angle astrophotos into tiles, plate-solves each tile, and integrates the WCS solutions into the full image. Enables SPCC on ultra-wide-angle images that ImageSolver cannot handle.</p>
  </a>
</div>
</div>
